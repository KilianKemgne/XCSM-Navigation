Neutralino.init();
let nameBasic = document.getElementById('nameBasic');
let emailBasic = document.getElementById('emailBasic');
let doBasic = document.getElementById('dobBasic');
...
filtre.addEventListener('click', () => {
    ...
    fetch(url).then((response) => {
        response.text().then((xml) =>{
            ...
            if(nameBasic.value !== undefined && nameBasic.value !== 0 && doBasic == undefined && emailBasic == undefined){
                let partie = parties.item(nameBasic.value)
                    var chemin = '';
